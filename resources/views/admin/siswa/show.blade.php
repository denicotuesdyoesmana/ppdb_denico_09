...
<div style="max-width: 1200px; margin: 0 auto;">
    ...
    <div class="detail-header">
        <div>
            <h1 class="detail-title">{{ $siswa->nama_lengkap }}</h1>
            <p class="detail-subtitle">NISN: {{ $siswa->nisn }}</p>
        </div>
    ...
    <div class="info-grid">
        ...
        <div class="info-card">
            ...
            <div class="photo-container">
                @if($siswa->foto)
                    <img src="{{ asset('storage/' . $siswa->foto) }}" alt="Foto {{ $siswa->nama_lengkap }}" class="photo-image">
                @else
        ...
        <div class="info-card">
            ...
            <div class="info-card-body">
                <div class="info-item">
                    ...
                    <span class="info-value"><strong>{{ $siswa->nama_lengkap }}</strong></span>
                </div>
                <div class="info-item">
                    <span class="info-label">NISN</span>
                    <span class="info-value"><strong>{{ $siswa->nisn }}</strong></span>
                </div>
                <div class="info-item">
                    <span class="info-label">NIK</span>
                    <span class="info-value">{{ $siswa->nik ?? '-' }}</span>
                </div>
                <div class="info-item">
    ...
    @if($pendaftaran)
                ...
                <div class="info-card-body">
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">
                        <div>
                            ...
                            <div style="color: #1f2937; font-weight: 600; font-size: 14px; margin-top: 6px;">{{ $pendaftaran->nomor_pendaftaran ?? '-' }}</div>
                        </div>
                        <div>
                            <div class="info-label">Gelombang</div>
                            <div style="margin-top: 6px;">
                                <span class="badge badge-primary">Gelombang {{ $pendaftaran->gelombang ?? '-' }}</span>
                            </div>
                        </div>
                        <div>
                            <div class="info-label">Tanggal Daftar</div>
                            <div style="color: #1f2937; font-weight: 600; font-size: 14px; margin-top: 6px;">{{ $pendaftaran->tanggal_daftar ? $pendaftaran->tanggal_daftar->format('d M Y') : '-' }}</div>
                        </div>
                        <div>
                            ...
                            <div style="margin-top: 6px;">
                                ...
                                @endphp
                                <span class="badge {{ $badgeClass }}">{{ $pendaftaran->statusPendaftaran?->label ?? 'Menunggu' }}</span>
                            </div>
                        </div>
                        <div>
                            <div class="info-label">Jurusan Pilihan 1</div>
                            <div style="color: #1f2937; font-weight: 600; font-size: 14px; margin-top: 6px;">{{ $pendaftaran->jurusanPilihan1?->nama_jurusan ?? '-' }}</div>
                        </div>
                        <div>
                            <div class="info-label">Jurusan Pilihan 2</div>
                            <div style="color: #1f2937; font-weight: 600; font-size: 14px; margin-top: 6px;">{{ $pendaftaran->jurusanPilihan2?->nama_jurusan ?? '-' }}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Actions -->
        @if($pendaftaran->statusPendaftaran?->kode === 'menunggu')
            <div class="info-card" style="border: 2px solid #e5e7eb;">
                ...
                <div class="info-card-body">
                    ...
                    <div class="action-buttons">
                        <button type="button" class="btn btn-success btn-accept" data-siswa-id="{{ $siswa->id }}" data-siswa-name="{{ $siswa->nama_lengkap }}">
                            <i class="fas fa-check-circle"></i> Terima
                        </button>
                        <button type="button" class="btn btn-danger btn-reject" data-siswa-id="{{ $siswa->id }}" data-siswa-name="{{ $siswa->nama_lengkap }}">
    ...
    <div class="info-card" style="border: 2px dashed #ef4444; background: rgba(239, 68, 68, 0.02);">
        ...
        <div class="info-card-body">
            ...
            <button type="button" class="btn btn-danger btn-reset-biodata" data-siswa-id="{{ $siswa->id }}" data-siswa-name="{{ $siswa->nama_lengkap }}">
