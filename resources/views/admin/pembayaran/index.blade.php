...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5">
        ...
        <a href="{{ route('admin.pembayaran.export') }}" class="btn btn-outline-success">
            ...
            <div class="card border-0 h-100 shadow-sm hover-shadow transition">
                <div class="card-body d-flex align-items-center">
                    <div class="flex-grow-1">
                        ...
                        <h3 class="mb-0 text-warning fw-bold">{{ $statusSummary['Menunggu Verifikasi'] }}</h3>
                    </div>
            ...
            <div class="card border-0 h-100 shadow-sm hover-shadow transition">
                <div class="card-body d-flex align-items-center">
                    <div class="flex-grow-1">
                        ...
                        <h3 class="mb-0 text-success fw-bold">{{ $statusSummary['Terverifikasi'] }}</h3>
                    </div>
            ...
            <div class="card border-0 h-100 shadow-sm hover-shadow transition">
                <div class="card-body d-flex align-items-center">
                    <div class="flex-grow-1">
                        ...
                        <h3 class="mb-0 text-danger fw-bold">{{ $statusSummary['Ditolak'] }}</h3>
                    </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('admin.pembayaran.index') }}" method="GET" class="row g-3">
                ...
                <div class="col-md-2 col-sm-6">
                    ...
                    <select name="metode" class="form-select form-select-sm border-0 bg-light">
                        ...
                        <option value="Transfer Bank" {{ request('metode') === 'Transfer Bank' ? 'selected' : '' }}>
                            Transfer Bank
                        </option>
                        <option value="E-Wallet" {{ request('metode') === 'E-Wallet' ? 'selected' : '' }}>
                            E-Wallet
                        </option>
                    </select>
                </div>
            ...
            <table class="table table-hover table-nowrap mb-0" id="pembayaranTable">
                ...
                <tbody>
                    @forelse($pembayarans as $pembayaran)
                        <tr class="pembayaran-row align-middle border-bottom" data-id="{{ $pembayaran->id }}">
                            <td class="ps-3">
                                <input type="checkbox" class="form-check-input pembayaran-checkbox cursor-pointer" value="{{ $pembayaran->id }}">
                            </td>
                            <td class="ps-2">
                                <span class="badge bg-secondary">{{ $pembayaran->id }}</span>
                            </td>
                            <td class="ps-2">
                                @if($pembayaran->pendaftaran && $pembayaran->pendaftaran->siswa)
                                    <div class="fw-bold text-dark">{{ $pembayaran->pendaftaran->siswa->nama_lengkap }}</div>
                                @else
                                    <span class="text-muted">-</span>
                                @endif
                            </td>
                            <td class="ps-2">
                                <div class="fw-500">{{ $pembayaran->nama }}</div>
                                @if($pembayaran->nama_bank)
                                    <small class="text-muted d-block">
                                        <i class="ti ti-building-bank me-1"></i>{{ $pembayaran->nama_bank }}
                                    </small>
                                @elseif($pembayaran->jenis_ewallet)
                                    <small class="text-muted d-block">
                                        <i class="ti ti-wallet me-1"></i>{{ $pembayaran->jenis_ewallet }}
                                    </small>
                                @endif
                            </td>
                            <td class="ps-2">
                                <span class="badge bg-light text-dark border border-light">{{ $pembayaran->metode }}</span>
                            </td>
                            <td class="ps-2">
                            ...
                            <td class="ps-2">
                                ...
                                <span class="badge bg-{{ $statusConfig['badge'] }}">
                                    <i class="ti {{ $statusConfig['icon'] }} me-1"></i>{{ $pembayaran->status }}
                                </span>
                            </td>
                            <td class="ps-2">
                                <div>{{ $pembayaran->created_at->format('d M Y') }}</div>
                                <small class="text-muted">{{ $pembayaran->created_at->format('H:i') }}</small>
                            </td>
                            <td class="text-center ps-1">
                                <div class="d-flex gap-2 justify-content-center flex-wrap">
                                    ...
                                    <button type="button" class="btn btn-danger btn-sm btn-delete-pembayaran" 
                                            data-id="{{ $pembayaran->id }}" title="Hapus" data-bs-toggle="tooltip">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performBulkAction(status) {
        ...
        Swal.fire({
            ...
            fetch('{{ route("admin.pembayaran.bulk-update") }}', {
                method: 'POST',
                headers: {
